object session {import scala.runtime.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(22); val res$0 = 
  1+2;System.out.println("""res0: Int(3) = """ + $show(res$0));$skip(53); 
  def abs(x: Double) : Double = if (x < 0) -x else x;System.out.println("""abs: (x: Double)Double""");$skip(126); 
	...
	def isGoodEnough(guess: Double, x: Double) =
		abs(guess * guess - x)  / x < 1e-6;System.out.println("""isGoodEnough: (guess: Double, x: Double)Boolean""");$skip(77); 
	
	def improve(guess: Double, x: Double) =
		(guess / 2) + (x / (2 * guess));System.out.println("""improve: (guess: Double, x: Double)Double""");$skip(41); 
	...
	sqrt(1e-60);System.out.println("""res3: Double = """ + $show(res$3));$skip(12); val res$4 = 
